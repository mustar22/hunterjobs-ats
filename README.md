...
<p align="center">
  <sub><strong>v0.8.5 just shipped</strong> - one button pulls down a few thousand companies I already researched plus YC/HN listings to judge, every model picker is live, and company domains now resolve on their own, so a search key is optional far more often. <a href="#changelog--roadmap">See changelog &darr;</a></sub>
</p>
...
- **Company domains resolve without a search call** - LinkedIn hands over a company name and nothing else, so 98% of newly-seen companies had no domain and every one of them burned a paid web search just to find a homepage. Names now resolve to a domain directly, and nothing is trusted until the page says the company's name back: a one-word company must own the domain label outright, and only `.com` is ever guessed, because `.com` ownership is itself the proof - a one-word company that owns `<word>.com` is almost certainly *the* holder of that name, while `.ai`/`.io`/`.co` is where the other real companies sharing it end up (`prima.ai` is a manufacturer, not the Prima that was hiring). 13 of 20 real cases resolve with no search call at all, which also means a search key is optional far more often
