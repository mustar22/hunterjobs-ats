...
- **hh listings resolve to a real company domain** - 0% before, 86% on a tech search now. The employer's own site rides along in the search result, so it costs nothing: I checked six employers that had none and their employer pages had none either, which means there is no second request worth making. Two cleanups that mattered on live data: some employers list `hh.ru` itself as their website, and plenty list an HR portal like `job.alfabank.ru`, which becomes `alfabank.ru` so research and email guessing aim at the company instead of its careers page
